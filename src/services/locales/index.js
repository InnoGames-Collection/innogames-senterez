export default {
  i18n: {
    es: {
      login: {
        title: 'Autenticar usuario',
        username: 'Usuario',
        password: 'Contraseña',
        authenticate: 'Autenticar',
        createaccount: 'Crear una cuenta',
        errors: {
          user_not_found: 'Usuario no encontrado',
          passw_incorrect: 'Contraseña incorrecta',
          consultando: 'Consultando'
        }
      },
      register: {
        title: 'Registrar usuario',
        username: 'Usuario',
        password: 'Contraseña',
        name: 'Nombre',
        email: 'Correo',
        firstName: 'Primer Apellido',
        lastName: 'Segundo Apellido',
        insert: 'Crear',
        cancel: 'Cancelar',
        mascul: 'Masculino',
        femen: 'Femenino',
        age: 'Edad',
        passwordRepit: 'Repetir Password',
        errors: {
          user_not_found: 'Usuario no encontrado',
          passw_incorrect: 'Contraseña incorrecta',
          consultando: 'Consultando'
        }
      },
      account: {
        personalData: {
          title: 'Datos personales',
          name: 'Nombre',
          username: 'Usuario',
          email: 'Correo',
          created: 'Creado',
          edit: 'Editar cuenta'
        },
        activity: {
          title: 'Actividad',
          vsPC: 'Stats against CPU',
          vsOtherUser: '2 Player Stats',
          wins: 'Ganadas',
          draws: 'Tablas',
          losses: 'Perdidos',
          whites: 'Blancas',
          blacks: 'Negras'
        }
      },
      home: {
        title: 'Inicio',
        online: 'Conectados',
        played: 'Jugando',
        upload: 'Archivo',
        invite: 'Invitar',
        send: 'Enviar',
        inviteSend: 'invitación de juego enviada',
        createPart: {
          title: 'Crear partida',
          time: 'Tiempo minutos',
          type: 'Tipo partida',
          create: 'Crear',
          cancel: 'Cancelar',
          black: 'Negras',
          white: 'Blancas',
          timePersonal: 'Establecer tiempo',
          fieldTime: 'Tiempo',
          public: 'Publica',
          private: 'Privada'
        },
        uploadMens: {
          success: 'Archivos subidos correctamente',
          path_not_permit: 'Extención de archivo no permitida',
          size_not_permit: 'Excede tamaño permitido'
        }
      },
      invites: {
        title: 'Invitaciones',
        numIn: 'Invitaciones de usuarios',
        vspc: 'Jugar contra la PC',
        initgame: 'Iniciando juego'
      },
      visor: {
        title: 'Visor',
        found: 'Encontrados',
        whiteName: 'Nombre blancas',
        blackName: 'Nombre negras',
        result: 'Resultado',
        wins: 'Gana',
        by: 'por'
      },
      puzzles: {
        title: 'Puzzles',
        create: {
          title: 'Crea un Puzzle',
          description: 'Adiciona y comparte un puzzle retador',
          create: 'Crear',
          delete: 'Eliminar',
          edit: 'Editar',
          cancel: 'Cancelar',
          add: 'Adicionando puzzle',
          fenInit: 'Posición inicial',
          fenFinish: 'Posición final'
        },
        list: 'Listado',
        types: {
          MateIn1: {
            name: 'Mate en 1',
            descript: 'Las blancas ganan en un movimiento'
          },
          MateIn2: {
            name: 'Mate en 2',
            descript: 'Las blancas ganan en dos movimientos'
          },
          MateIn3: {
            name: 'Mate en 3',
            descript: 'Las blancas ganan en tres movimientos'
          },
          FindFork: {
            name: 'Encontrar tenedor',
            descript: 'Buscar movimiento blanco que da como resultado un tenedor'
          },
          TakePiece: {
            name: 'Toma una pieza',
            descript: 'Una de las piezas negras no está defendida. Tómala'
          }
        },
        attention: 'Atención',
        attMen: 'Selecciona un tipo de puzzle a resolver',
        createBy: 'Creado por',
        date: 'Fecha',
        played: 'Juega',
        movs: 'Movimientos restantes',
        wins: 'Ganas',
        lost: 'Pierdes',
        reintent: 'Reintentar',
        intents: 'Intentos',
        color: {
          black: 'Negras',
          white: 'Blancas'
        }
      },
      about: {
        title: 'Acerca de',
        description: 'Acerca de la Aplicación',
        namegame: 'Senterez ሰንጠረዥ',
        seemore: 'Código fuente',
        des: 'Plataforma de ajedrez multijugador en línea. Juega contra otros jugadores en tiempo real o contra la computadora. Resuelve puzzles y crea nuevos retos.',
        tecn: 'Tecnologías',
        dev: 'Desarrollador',
        developer: 'InnoSphere Technologies — en asociación con Ethio Telecom',
        license: 'Licencia',
        licenseText: 'Software libre bajo GNU AGPL v3.0. Ver LICENSE y NOTICE en el repositorio.'
      },
      game: {
        title: 'Juego',
        chat: 'Chat del juego',
        rend: 'Rendir',
        chatConverTooltip: 'Selecciona un usuario conectado',
        chatConver: 'Conversar con',
        pieces: 'Piesas',
        board: 'Tablero',
        invert: 'Invertir',
        text: 'Texto',
        check: 'Jaque a las',
        descriptiontext: 'Enviar mensaje',
        turnColor: 'Juegan las',
        wins: 'Gana',
        incheck: 'En jaque',
        motives: {
          time: 'por tiempo',
          rendi: 'por rendición',
          login: 'Autenticarse'
        }
      },
      user: {
        title: 'Usuario',
        logout: 'Salir',
        login: 'Autenticarse'
      }
    },
    en: {
      login: {
        title: 'user Login',
        username: 'Username',
        password: 'Password',
        authenticate: 'Login',
        createaccount: 'Create account',
        errors: {
          user_not_found: 'user not found',
          passw_incorrect: 'Incorrect password',
          consultando: 'Consulting'
        }
      },
      register: {
        title: 'Register user',
        username: 'Username',
        password: 'Password',
        name: 'Name',
        email: 'Email',
        firstName: 'Surname',
        lastName: 'Second surname',
        insert: 'Insert',
        cancel: 'Cancel',
        mascul: 'Male',
        femen: 'Female',
        age: 'Age',
        passwordRepit: 'Repeat Password',
        errors: {
          user_not_found: 'User not found',
          passw_incorrect: 'Incorrect password',
          consultando: 'Consulting'
        }
      },
      account: {
        personalData: {
          title: 'Personal data',
          name: 'Name',
          username: 'Username',
          email: 'Email',
          created: 'Created',
          edit: 'Edit account'
        },
        activity: {
          title: 'Activity',
          vsPC: 'Stats against CPU',
          vsOtherUser: '2 Player Stats',
          wins: 'Wins',
          draws: 'Draws',
          losses: 'Losses',
          whites: 'Whites',
          blacks: 'Blacks'
        }
      },
      home: {
        title: 'Home',
        online: 'Online',
        played: 'Played',
        upload: 'File',
        invite: 'Invite',
        send: 'Send',
        inviteSend: 'Game invitation sent',
        createPart: {
          title: 'Create match',
          time: 'Time minutes',
          type: 'Match type',
          create: 'Create',
          cancel: 'Cancel',
          black: 'Black',
          white: 'White',
          timePersonal: 'Personal time',
          fieldTime: 'Time',
          public: 'public',
          private: 'private'
        },
        uploadMens: {
          success: 'Files Uploaded Correctly',
          path_not_permit: 'File extension not allowed',
          size_not_permit: 'Exceeds allowed size'
        }
      },
      invites: {
        title: 'Invitations',
        numIn: 'Users invitations',
        vspc: 'Play against PC',
        initgame: 'Starting game'
      },
      visor: {
        title: 'Viewer',
        found: 'Found',
        whiteName: 'White name',
        blackName: 'Black name',
        result: 'Result',
        wins: 'Wins',
        by: 'by'
      },
      puzzles: {
        title: 'Puzzles',
        create: {
          title: 'Created a Puzzle',
          description: 'Add and share a challenging puzzle',
          create: 'Created',
          delete: 'Delete',
          edit: 'Edit',
          cancel: 'Canel',
          add: 'Adding puzzle',
          fenInit: 'Initial position',
          fenFinish: 'Finish position '
        },
        list: 'List',
        types: {
          MateIn1: {
            name: 'Mate in 1',
            descript: 'White wins in one move'
          },
          MateIn2: {
            name: 'Mate in 2',
            descript: 'White wins in two moves'
          },
          MateIn3: {
            name: 'Mate in 3',
            descript: 'White wins in three moves'
          },
          FindFork: {
            name: 'FindFork',
            descript: 'Find white move that results in fork'
          },
          TakePiece: {
            name: 'TakePiece',
            descript: 'Find white move that results in fork'
          }
        },
        attention: 'Attention',
        attMen: 'Select the puzzle to resolver',
        createBy: 'Create by',
        date: 'Date',
        played: 'Played',
        movs: 'Remaining moves',
        wins: 'Wins',
        lost: 'Lost',
        reintent: 'Reintent',
        intents: 'Intents',
        color: {
          black: 'black',
          white: 'white'
        }
      },
      about: {
        title: 'About',
        description: 'About the application',
        namegame: 'Senterez ሰንጠረዥ',
        seemore: 'Source code',
        des: 'Online multiplayer chess platform. Play against other players in real time or against the computer. Solve puzzles and create new challenges.',
        tecn: 'Technologies',
        dev: 'Developer',
        developer: 'InnoSphere Technologies — in partnership with Ethio Telecom',
        license: 'License',
        licenseText: 'Open source under GNU AGPL v3.0. See LICENSE and NOTICE in the repository.'
      },
      game: {
        title: 'Game',
        chat: 'Game´s chat',
        chatConver: 'converse with',
        chatConverTooltip: 'Select a user online',
        rend: 'surrend',
        pieces: 'Pieces',
        board: 'Board',
        invert: 'Invert',
        text: 'Text',
        descriptiontext: 'Send message',
        turnColor: 'Played ',
        wins: 'Wins',
        incheck: 'in jaque',
        motives: {
          time: 'by time',
          rendi: 'by capitulation',
          login: 'Login'
        }
      },
      user: {
        title: 'User',
        logout: 'Exit',
        login: 'Login'
      }
    },
    am: {
      login: {
        title: 'መግቢያ',
        username: 'የተጠቃሚ ስም',
        password: 'የይለፍ ቃል',
        authenticate: 'ግባ',
        createaccount: 'መለያ ፍጠር',
        errors: {
          user_not_found: 'ተጠቃሚ አልተገኘም',
          passw_incorrect: 'የይለፍ ቃል ትክክል አይደለም',
          consultando: 'በመጫን ላይ'
        }
      },
      about: {
        title: 'ስለ',
        description: 'ስለ Senterez',
        namegame: 'Senterez ሰንጠረዥ',
        seemore: 'የምንጭ ኮድ',
        des: 'በመስመር ላይ የሚጫወቱ ብዙ ተጫዋቾች የሻማ ጨዋታ። ከሌሎች ተጫዋቾች ወይም ከኮምፒዩተር ጋር በቀጥታ ይጫወቱ። ፓዝሎችን ይፈቱ እና አዲስ ፈተናዎችን ይፍጠሩ።',
        tecn: 'ቴክኖሎጂ',
        dev: 'ገንቢ',
        developer: 'InnoSphere Technologies — በ Ethio Telecom ግንኙነት',
        license: 'ፈቃድ',
        licenseText: 'GNU AGPL v3.0 ነፃ ሶፍትዌር። LICENSE እና NOTICE በማከማቻ ውስጥ ይመልከቱ።'
      },
      home: { title: 'መነሻ' },
      game: { title: 'ጨዋታ' },
      user: { title: 'ተጠቃሚ', logout: 'ውጣ', login: 'ግባ' },
      invites: { title: 'ግብዣዎች' },
      visor: { title: 'ተመልካች' },
      puzzles: { title: 'ፓዝሎች' }
    }
  }
}
